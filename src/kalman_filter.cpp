...
Eigen::VectorXd KalmanFilter::PredictionToObservation(
...
void KalmanFilter::Update(const Eigen::VectorXd& z) {
    ...
    // Joseph form
    // P_ = (I - K * H_) * P_predict_ * (I - K * H_).transpose() + K * R_ *
    // K.transpose();
    // Optimal gain
    P_ = (I - K * H_) * P_predict_;
}

float KalmanFilter::CalculateLogLikelihood(const Eigen::VectorXd& y,
                                           const Eigen::MatrixXd& S) {
    float log_likelihood;

    // Note: Computing log(M.determinant()) in Eigen C++ is risky for large
    // matrices since it may overflow or underflow. compute the Cholesky
    // decomposition of the innovation covariance matrix, because it is
    // symmetric S = L * L^T = U^T * U then retrieve factor L in the
    // decomposition
    auto& L = S.llt().matrixL();

    // find log determinant of innovation covariance matrix
    float log_determinant = 0;
    for (unsigned int i = 0; i < S.rows(); i++) log_determinant += log(L(i, i));
    log_determinant *= 2;

    // log-likelihood expression for current iteration
    log_likelihood = -0.5 * (y.transpose() * S.inverse() * y +
                             num_obs_ * log(2 * M_PI) + log_determinant);
